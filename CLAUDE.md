...
- `src/agents/agent-router.ts` — `AgentRouter`, wired into `APIServer` via `GET /api/v1/agents/:intent`. Deterministic dispatch only — the caller passes an explicit `AgentIntent` enum value (`wallet_overview`, `transaction_lookup`, `wallet_risk`, `wallet_evidence`, `wallet_alerts`, `research_report`, `market_events`); there is no NLP/LLM step guessing intent from free text. A `switch` with an exhaustiveness check (`const _exhaustive: never = intent`) ensures a new `AgentIntent` value fails to compile here until handled.
