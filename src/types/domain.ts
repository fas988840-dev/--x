...
export function validateWalletAddress(addr: string): WalletAddress {
  try {
    ...
    // ValidationError, not plain Error: APIServer's centralized error
    // handler switches on `instanceof ValidationError` to answer 400
    // rather than 500 - a plain Error here (the bug this fixes) meant
    // every invalid-input request across the whole API returned 500,
    // undetected until npm test first ran for real on this branch.
